...
//spi1在APB2总线上(72MHz),SPI最高频率是18MHz;
//spi2,spi3在APB1总线上(36MHz),SPI最高频率是18MHz;
...
void SPI1_Init(void)
{
    ...
    SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;                          //NSS信号由硬件（NSS管脚）还是软件（使用SSI位）管理:硬件控制SS输出
    SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_8; ///定义波特率预分频的值:波特率预分频值为8 SPI频率为9MHz（72/8）
...
void SPI2_Init(void)
{
    ...
    SPI_InitStructure.SPI_CPHA = SPI_CPHA_1Edge;                       //数据捕获于第1个时钟沿
    SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;                          //NSS信号由硬件（NSS管脚）还是软件（使用SSI位）管理:硬件控制SS输出
    SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4; //定义波特率预分频的值:波特率预分频值为4 SPI频率为9MHz（36/4）
    SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;                 //指定数据传输从MSB位还是LSB位开始:数据传输从MSB位开始
    SPI_InitStructure.SPI_CRCPolynomial = 7;                           //CRC值计算的多项式
